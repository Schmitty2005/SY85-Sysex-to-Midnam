...
# Hex for LSB of v1, v2,... ...p1, p2
BANK_CODE = ("00", "03", "06", "09", "64", "67")
BANKS_VERBOSE = ("Voice 1", "Voice 2", "Voice 3", "Voice 4",
                 "Performance 1", "Performance 2")
BANKS = BANKS_VERBOSE
# needs bank prefix and bank suffix from v1, v2, etc.
PATCHBANK_HEADER = "<PatchBank Name=\"" + patchxmlinfo + "\">"


# [g1]             BANKS_VERBOSE + BANK_PREFIX #Between voice groups
CUBASE_BANK_HEADER = "[g1]             "
# Example  : [p2, x, 32, b]  x = patch number, b = bank code (0, 3, 6, 9, 64, 67)
CUBASE_PATCH_PREFIX = "[p2, "
cubaseOutput = list()
...
BANK_SLICES = (voice1slice, voice2slice, voice3slice,
               voice4slice, perf1slice, perf2slice)
...
# save Cubase device file .txt


def savecubase(cubaseArray, outputfile):
    output = ""
    for y in cubaseArray:
        output = output + (y) + "\n"
    savemidnam(output, outputfile)
    return 1
...
def PatchNamesToXML(patchesList):
    ...
    for x in patchesList:
        output = "<Patch Number=\""+(str(counter+1).zfill(2))+"\" Name=\"" + \
            patchesList[counter]+"\" ProgramChange=" + \
            str(counter)+"/>"  # Change=0 needs fixing
        print(output)
        counter = counter + 1
...
def BankPrint(sysexfilename):  # main meat and potatoes
    ...
    for x in BANKS:
        ...
        cubasepathheader = CUBASE_BANK_HEADER + x
        cubaseOutput.append(cubasepathheader)
        # Add Cubase [g1] thing here
        #
        for patch in patchlist[BANK_SLICES[bankcounter]]:
            needs = pnum[patchnumber - 1]+"\"" + " Name=\"" + \
                pnum[patchnumber-1] + ": "+patch + " "
            ...
            xmlout = "     <Patch Number=\"" + needs + \
                "\" ProgramChange=\"" + str(patchnumber-1)+"\"/>\n"
            cubasepatch = CUBASE_PATCH_PREFIX + \
                str(patchnumber-1) + ", 32, " + \
                BANK_CODE[bankcounter] + "]        " + patch  # + "\n"
            patchesXMLoutput.append(xmlout)
            cubaseOutput.append(cubasepatch)
    ...
    for x in BANKS:
        ...
        lsbBankswitch = ("")
        header1 = "\n\n\n<PatchBank Name = \"" + \
            BANK_PREFIX+" " + BANKS[counter]+"\">\n"
        Mcommands = "  <MIDICommands>\n"
        cchangemsb = "     <ControlChange Control = \"0\" Value = \"0\"/>\n"
        cchangelsb = "     <ControlChange Control = \"32\" Value = \"" + \
            BANK_CODE[counter]+"\" />\n"  
        Mccomandsend = "</MIDICommands>\n"
        patchnamestart = "  <PatchNameList>\n"
        bankheader = header1 + Mcommands + cchangemsb + \
            cchangelsb + Mccomandsend + patchnamestart
...
savecubase(cubaseOutput, cubaseoutfile)
